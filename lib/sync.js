        ...
        if (exists) {
            readFile(path.resolve(process.cwd(), configFile), function(rawConfig) {
                ...
                if (_.isObject(config['sync'])) {
                    ...
                    } else if (syncProvider === "url") {
                        var syncConfig = config['sync']['config'];
                        ...
                        stringFileLocator(srcPath, function(stringFiles) {
                            ...
                            Object.keys(translations).forEach(function (language) {
                                ...
                                fetch(url).then(function (response) {
                                    ...
                                    // TODO: remove when steven fixes backend
                                    return raw.replace(/\n/g, ";\n") + ";"
                                }).then(function (textContent) {
                                    return i18nStrings.parse(textContent);
                                }).then(function (translatedContent) {
                                    ...
                                    if (counter === 0) {
                                        ...
                                        updateStrings(srcPath, function(currentTranslations, locale, updateDataCB) {
                                            ...
                                            if (_.isEmpty(updates)) {
                                                ...
                                                if (options.updateFiles === true) {
                                                    updateDataCB(currentTraconslations);
                                                } else {
                                                    shouldExitWithError = true;
                                                }
                                            }
