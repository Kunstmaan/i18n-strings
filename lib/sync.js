        ...
        if (exists) {
            readFile(path.resolve(process.cwd(), configFile), function(rawConfig) {
                var config = JSON.parse(rawConfig),
                    sheet, creds, keyColumn, valueColumns;

                if (config.hasOwnProperty('google')) {
                    if (config['google'].hasOwnProperty('translation-file') && config['google'].hasOwnProperty('creds')) {
                        sheet = new GS(config['google']['translation-file']);
                        creds = config['google']['creds'];
                    
                        if (config['google'].hasOwnProperty('columns')) {
                            if (config['google']['columns'].hasOwnProperty('key')) {
                                keyColumn = config['google']['columns']['key'].replace(/\s+/g, '').toLowerCase();
                            } else {
                                keyColumn = 'ioskey';
                            }

                            if (config['google']['columns'].hasOwnProperty('value')) {
                                valueColumns = _.map(config['google']['columns']['value'], function(valueColumn) {
                        ...
                        throw Error('Invalid google configuration.');
                    }
                } else {
                    throw Error('No configuration found for google docs.');
                }
            });
        } else {
