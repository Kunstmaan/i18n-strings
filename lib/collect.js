...
var REGEXES = {
    "swift": {
        collecting: [/i18nNavKey[^}]+return\s+"([^"]+)"/gmi, /i18nTabKey[^}]+return\s+"([^"]+)"/gmi, /"([^"]+)"\.i18nLocalized/gi, /localizedStringForKey\("([^"]+)"[^\)]+\)/gi, /localizedString\(forKey:"([^"]+)"[^\)]+\)/gi],
        alerts: [
            {
    ...
    "storyboard": {
        collecting: [/.*<userDefinedRuntimeAttribute[^\>]*keyPath="i18n[^"]+"[^\>]*value="([^"]+)"[^\>]*>/gi],
        alerts: [
            {
